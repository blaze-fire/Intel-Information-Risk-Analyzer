...
#preprocess text (removing stopwords, lemmtizing)
...
# processing text for risk type, country and month
def preprocess_text_for_type_and_month(info):
        ...
        if filenames:
            for i in filenames:
                with open(dirpath + '/' + i) as f:
                    ...
                    #otherwise read the entire text to understand the context
                    else:
                        for key in keyword:
                            if key in info:
                                categories.append(i.split('.')[0])
...
def type_of_alert(info):
    ...
    for word in info:
        ...
        elif 'internationalsos' in word or 'riskinsights' in word:
            type_risk = 'ISOS'
            break
...
def extract_country_and_month(text, info):
    country_dict = {}
    ...
    # storing all the countries present in text in a dictionary and selecting the one which occurs first
    # in text
    for i in countries:
        if i in text:
            country_dict[text.find(i)] = i

    min_index = min(country_dict.keys())
    country = country_dict[min_index]

    for word in info:
        if word in ['april', 'august', 'december', 'february', 'january', 'july', 'june', 'march', 'may', 'november', 'october', 'september']:
            month = word
            break
    try:
...
#updating value of scores
def update_scores(df, info, country, month):

    df = df.reset_index()

    #extracting importance ['high']
    ...
    #extracting severity ['notice', 'advisory']
    if 'level' in info:
...
#updating value of risks
def update_risks(df, risks, country, month):

    df = df.reset_index()

    #updating risks according to country and month
    for risk in risks:
...
def plot_risk_meter(df, country, month):
    ...
    # as 10 risk types, therefore for 10 risk meters 10 traces 
...
def plot_risk_trend(df, country, choice):
    ...
    fig.update_layout(
        ...
        # Drawing horizontal line and setting arbitrary threshold for now 1.5
        shapes=[dict(
            type='line',
            line_color="#F3E40F",
...
#for ocr of uploaded images using azure
def azure_ocr(img):
    ...
    # to store data from json format to continous string 
    data = ''
                ...
                if docx_file is not None:
                    ...
                    if docx_file.type == "text/plain":
                        ...
                        try:
                            ...
                            text_for_cat_and_month = preprocess_text_for_type_and_month(text)

                            processed_text = preprocess_pdf_text(text)
                            categories = select_category(processed_text)
                            st.write(categories)
                            
                            alert_type = type_of_alert(text_for_cat_and_month)
                            st.write("Type of alert: " + alert_type)
                            
                            country, month = extract_country_and_month(text, text_for_cat_and_month)
            ...
            for image_file in image_files:
                if image_file is not None:
                    ...
                    try:
                        ...
                        text_for_cat_and_month = preprocess_text_for_type_and_month(ocr_text)

                        categories = select_category(processed_text)
                        st.write(categories)
                        
                        country, month = extract_country_and_month(ocr_text, text_for_cat_and_month)
                        st.write("Country: " + country)
                        st.write("Month: " + month)
